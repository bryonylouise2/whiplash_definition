...
with imageio.get_writer('/home/bpuxley/Definition_and_Climatology/Plots/Case_Study/Drought-to-Pluvial/%s_spi_drought.gif'%(event_num), mode='I') as writer:
...
for i in tqdm(range(0,len(event_dates))):
	date = event_dates.iloc[i]
	pluvial_date = (pd.to_datetime(event.Whiplash_Date.iloc[i]) + timedelta(days=30)).strftime('%Y-%m-%d')
	...
	cs = plt.contourf(lons, lats, spi.spi_30day.sel(time=pluvial_date).values, transform=ccrs.PlateCarree(), extend = "both", levels=np.arange(-3, 3, 0.1), cmap = 'BrBG') 
...
with imageio.get_writer('/home/bpuxley/Definition_and_Climatology/Plots/Case_Study/Drought-to-Pluvial/%s_spi_pluvial.gif'%(event_num), mode='I') as writer:
    for filename in pathfiles:
        image = imageio.imread(filename)
        writer.append_data(image)
...
for i in tqdm(range(0,len(event_dates))):
	date = event_dates.iloc[i]
	drought_date = event.Whiplash_Date.iloc[i]
	pluvial_date = (pd.to_datetime(event.Whiplash_Date.iloc[i]) + timedelta(days=30)).strftime('%Y-%m-%d')
...
with imageio.get_writer('/home/bpuxley/Definition_and_Climatology/Plots/Case_Study/Drought-to-Pluvial/%s_spi_change.gif'%(event_num), mode='I') as writer:
    for filename in pathfiles:
        image = imageio.imread(filename)
        writer.append_data(image)
...
for i in tqdm(range(0,len(event_dates))):
	...
	cs = ax1.pcolormesh(lons, lats, whiplashes.DP_whiplashes.sel(time=whiplash_date), cmap=cmap, transform=ccrs.PlateCarree())
...
with imageio.get_writer('/home/bpuxley/Definition_and_Climatology/Plots/Case_Study/Drought-to-Pluvial/%s_whiplash_occurrences.gif'%(event_num), mode='I') as writer:
    for filename in pathfiles:
        image = imageio.imread(filename)
        writer.append_data(image)
